...
void Road::initialize(Vektoria::CScene& scene, const Vektoria::CHVector& pos)
{
	...
	auto wayType = m_params.wayType;
	auto crossingType = m_params.crossingType;

	m_infra.SetMaterialForWay(wayType, m_roadMat);
	m_infra.SetMaterialForCrossing(crossingType, m_crossingMat);
...
void Road::addQuarterCircle(Vektoria::CHVectors& line, float radius, float height, float startOffset, float direction) const
{
